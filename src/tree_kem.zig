...
pub const PathSecret = struct {
    ...
    pub fn deriveKeyPair(self: PathSecret, allocator: Allocator) !struct {
        ...
        var key_pair = try generateHpkeKeyPair(allocator, self.cipher_suite);
...
fn hpkeEncrypt(
    ...
    var client_ctx = try suite.createClientContext(
    ...
    const ciphertext_len = plaintext.len + suite.aead.tagLength();
    const ciphertext = try allocator.alloc(u8, ciphertext_len);
    defer allocator.free(ciphertext);

    // Encrypt
    try client_ctx.context.encryptToServer(plaintext, ciphertext, aad);

    // Get encapsulated key
    const enc = client_ctx.encapsulated_secret;
...
fn hpkeDecrypt(
    ...
    const plaintext_len = ciphertext.ciphertext.len() - suite.aead.tagLength();
    const plaintext = try allocator.alloc(u8, plaintext_len);
    errdefer allocator.free(plaintext);

    // Decrypt
    try server_ctx.decryptFromClient(ciphertext.ciphertext.asSlice(), plaintext, aad);
